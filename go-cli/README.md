# mixpanel-annotations-cli

boru-driven command-line client **and** interactive REPL for the MixpanelAnnotations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-annotations-cli)
make build

# 2. See usage (words, entities, env vars)
./mixpanel-annotations-cli --help

# 3. Provide credentials once, via the environment
export MIXPANEL_ANNOTATIONS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./mixpanel-annotations-cli load 1 annotation            # {id:1} shorthand
./mixpanel-annotations-cli load '{id:1}' annotation       # explicit match map
./mixpanel-annotations-cli update '{name:"x"}' annotation

# 5. Override the API base URL for a single call
MIXPANEL_ANNOTATIONS_BASE=https://api.example.com ./mixpanel-annotations-cli load 1 annotation

# 6. No arguments -> interactive REPL
./mixpanel-annotations-cli
mixpanel-annotations> load 1 annotation
mixpanel-annotations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-annotations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mixpanel-annotations-cli load 1 annotation
   ```
...
```sh
./mixpanel-annotations-cli load 1 annotation          # scalar shorthand for {id:1}
./mixpanel-annotations-cli load '{id:1}' annotation     # explicit match map
```
...
```sh
./mixpanel-annotations-cli update '{id:1,name:"new"}' annotation
```
...
./mixpanel-annotations-cli load 1 annotation
```
...
Run with no arguments to open a REPL (prompt `mixpanel-annotations>`). Each line is
evaluated as its own boru expression:

```text
$ ./mixpanel-annotations-cli
mixpanel-annotations> load 1 annotation
mixpanel-annotations> /help
mixpanel-annotations> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mixpanel-annotations-cli`. |
...
(`./mixpanel-annotations-cli <expr>`) and interactively (the REPL), and expressions compose the
