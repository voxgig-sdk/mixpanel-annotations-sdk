// Command MixpanelAnnotations-cli is a CLI + REPL for the MixpanelAnnotations SDK,
...
const prompt = "mixpanel-annotations"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "annotation annotation_tag list_annotation project tag"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MIXPANEL_ANNOTATIONS_APIKEY (api key), MIXPANEL_ANNOTATIONS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MIXPANEL_ANNOTATIONS_APIKEY carries the API key and
	// MIXPANEL_ANNOTATIONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MIXPANEL_ANNOTATIONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MIXPANEL_ANNOTATIONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMixpanelAnnotationsSDK(opts)
