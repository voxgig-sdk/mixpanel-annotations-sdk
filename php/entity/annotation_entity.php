...
// MixpanelAnnotations SDK Annotation entity
...
class AnnotationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AnnotationEntity($this->_client, $opts);
    }

    /**
     * @param Annotation|array $args Annotation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MixpanelAnnotationsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Annotation|array The current Annotation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Annotation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MixpanelAnnotationsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Annotation fields).
     */
     ...
     *
     * @param AnnotationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AnnotationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Annotation|array The loaded Annotation as an assoc-array at the
     *   SDK boundary; throws MixpanelAnnotationsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MixpanelAnnotationsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param AnnotationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AnnotationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Annotation|array The created Annotation as an assoc-array at the
     *   SDK boundary; throws MixpanelAnnotationsError on failure (item-5 convention).
     */
     ...
     *
     * @param AnnotationUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AnnotationUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Annotation|array The updated Annotation as an assoc-array at the
     *   SDK boundary; throws MixpanelAnnotationsError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Annotation matching the given criteria.
     *
     * @param AnnotationRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; AnnotationRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Annotation|array The removed Annotation as an assoc-array at the
     *   SDK boundary; throws MixpanelAnnotationsError on failure (item-5 convention).
     */
