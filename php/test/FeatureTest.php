...
// MixpanelAnnotations SDK feature test
//
...
class FtCtrl extends MixpanelAnnotationsControl
{
...
class FtClient
{
    ...
    private MixpanelAnnotationsUtility $utility;

    public function __construct(MixpanelAnnotationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): MixpanelAnnotationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MixpanelAnnotationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MixpanelAnnotationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MixpanelAnnotationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MixpanelAnnotationsConfig::make_feature($name);
    ...
    private static function buildUrl(MixpanelAnnotationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MixpanelAnnotationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MixpanelAnnotationsResponse($fetched);
        }
    ...
    private function fail(MixpanelAnnotationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MixpanelAnnotationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
