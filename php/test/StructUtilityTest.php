...
// MixpanelAnnotations SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MixpanelAnnotationsOmni::makeStructRunner(
    ...
    {
        return MixpanelAnnotationsOmni::objmodel(MixpanelAnnotationsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MixpanelAnnotationsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MixpanelAnnotationsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MixpanelAnnotationsOmni::structfix($now, Struct::undef()))
                        !== json_encode(MixpanelAnnotationsOmni::structfix($val, Struct::undef()))) {
