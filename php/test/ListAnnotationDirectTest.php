...
class ListAnnotationDirectTest extends TestCase
{
    public function test_direct_list_list_annotation(): void
    {
        $setup = list_annotation_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-list_annotation", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function list_annotation_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new MixpanelAnnotationsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
