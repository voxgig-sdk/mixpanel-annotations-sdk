...
class MixpanelAnnotationsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new MixpanelAnnotationsUtility();
        $this->_utility = $utility;

        $config = MixpanelAnnotationsConfig::shared_config();
        ...
        $feature_opts = MixpanelAnnotationsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MixpanelAnnotationsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MixpanelAnnotationsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MixpanelAnnotationsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MixpanelAnnotationsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MixpanelAnnotationsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MixpanelAnnotationsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MixpanelAnnotationsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MixpanelAnnotationsError($op . "_allow",
                "MixpanelAnnotationsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MixpanelAnnotationsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MixpanelAnnotationsError("graphql_error",
                "MixpanelAnnotationsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_annotation_tag = null;

    // Canonical facade: $client->AnnotationTag()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->annotation_tag()
    // resolves here too.
    public function AnnotationTag($data = null)
    {
        require_once __DIR__ . '/entity/annotation_tag_entity.php';
        if ($data === null) {
            if ($this->_annotation_tag === null) {
                $this->_annotation_tag = new AnnotationTagEntity($this, null);
            }
            return $this->_annotation_tag;
        }
        return new AnnotationTagEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new MixpanelAnnotationsSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
