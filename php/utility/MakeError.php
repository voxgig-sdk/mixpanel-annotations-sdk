...
class MixpanelAnnotationsMakeError
{
    public static function call(?MixpanelAnnotationsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MixpanelAnnotationsContext([], null);
        }
        $op = $ctx->op ?? new MixpanelAnnotationsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MixpanelAnnotationsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MixpanelAnnotationsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MixpanelAnnotationsError('', $msg, $ctx);
        ...
        if ($err instanceof MixpanelAnnotationsError) {
