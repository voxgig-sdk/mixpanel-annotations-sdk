# mixpanel-annotations-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MixpanelAnnotations SDK as
two agent tools — `mixpanel-annotations_list` and `mixpanel-annotations_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mixpanel-annotations-mcp)
make build

# 2. Provide credentials via the environment
export MIXPANEL_ANNOTATIONS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mixpanel-annotations \
  -- /absolute/path/to/mixpanel-annotations-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mixpanel-annotations-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mixpanel-annotations_list: first page of records
{ "entity": "annotation_tag" }
{ "entity": "annotation_tag", "query": { } }

// mixpanel-annotations_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mixpanel-annotations-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mixpanel-annotations \
     -- "$PWD"/dist/*/mixpanel-annotations-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mixpanel-annotations_list` and `mixpanel-annotations_load` tools now appear
   in new sessions. Ask the agent to *"list annotation_tag using mixpanel-annotations"*
   and it calls `mixpanel-annotations_list` with `{"entity":"annotation_tag"}`.
...
```sh
export MIXPANEL_ANNOTATIONS_APIKEY=sk_live_xxx            # API key
export MIXPANEL_ANNOTATIONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mixpanel-annotations_list` tool
...
### Call the `mixpanel-annotations_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mixpanel-annotations_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mixpanel-annotations_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MIXPANEL_ANNOTATIONS_APIKEY` | API key sent with every request. |
| `MIXPANEL_ANNOTATIONS_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

annotation | annotation_tag | list_annotation | project | tag

### Smoke test via HTTP (raw JSON-RPC)

```sh
./mixpanel-annotations-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mixpanel-annotations_load","arguments":{"entity":"annotation","query":{"id":1}}}}'
```
