...
from mixpanelannotations_sdk.core.control import MixpanelAnnotationsControl
...
from mixpanelannotations_sdk.core.helpers import get_ctx_prop, to_map
...
class MixpanelAnnotationsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = MixpanelAnnotationsControl()
        ...
        if isinstance(sp, MixpanelAnnotationsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, MixpanelAnnotationsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, MixpanelAnnotationsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return MixpanelAnnotationsOperation({})
        ...
        op = MixpanelAnnotationsOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return MixpanelAnnotationsError(code, msg, self)
