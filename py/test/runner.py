# MixpanelAnnotations SDK test runner
...
class MixpanelAnnotationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MixpanelAnnotationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MixpanelAnnotationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MixpanelAnnotationsTestRunner.getenv("MIXPANEL_ANNOTATIONS_TEST_LIVE")
        override = MixpanelAnnotationsTestRunner.getenv("MIXPANEL_ANNOTATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MixpanelAnnotationsTestRunner.getenv(key)
        ...
        explain = MixpanelAnnotationsTestRunner.getenv("MIXPANEL_ANNOTATIONS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MIXPANEL_ANNOTATIONS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MixpanelAnnotationsTestRunner._test_control is not None:
            return MixpanelAnnotationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MixpanelAnnotationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MixpanelAnnotationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MixpanelAnnotationsTestRunner.load_test_control()
...
def load_env_local():
    MixpanelAnnotationsTestRunner.load_env_local()


def env_override(m):
    return MixpanelAnnotationsTestRunner.env_override(m)


def entity_data(v):
    return MixpanelAnnotationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MixpanelAnnotationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MixpanelAnnotationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MixpanelAnnotationsTestRunner.load_test_control()


def live_client_options():
    return MixpanelAnnotationsTestRunner.live_client_options()


def live_delay_ms():
    return MixpanelAnnotationsTestRunner.live_delay_ms()
