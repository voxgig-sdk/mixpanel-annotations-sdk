...
class TestAnnotationDirect:
    ...
    def test_should_direct_load_annotation(self):
        setup = _annotation_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-annotation", "live" if setup["live"] else "unit")
        if _skip:
...
def _annotation_direct_setup(mockres):
