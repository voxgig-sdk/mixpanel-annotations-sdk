...
class TestListAnnotationDirect:

    def test_should_direct_list_list_annotation(self):
        setup = _list_annotation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-list_annotation", "live" if setup["live"] else "unit")
        if _skip:
...
def _list_annotation_direct_setup(mockres):
    ...
    live = env.get("MIXPANEL_ANNOTATIONS_TEST_LIVE") == "TRUE"
    ...
    if live:
        ...
        client = MixpanelAnnotationsSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
