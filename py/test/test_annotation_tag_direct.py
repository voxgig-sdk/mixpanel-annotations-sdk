...
class TestAnnotationTagDirect:

    def test_should_direct_list_annotation_tag(self):
        setup = _annotation_tag_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-annotation_tag", "live" if setup["live"] else "unit")
        if _skip:
...
def _annotation_tag_direct_setup(mockres):
