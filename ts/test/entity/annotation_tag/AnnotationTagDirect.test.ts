...
describe('AnnotationTagDirect', async () => {
  ...
  test('direct-list-annotation_tag', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-annotation_tag', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MIXPANEL_ANNOTATIONS_TEST_ANNOTATION_TAG_ENTID']
