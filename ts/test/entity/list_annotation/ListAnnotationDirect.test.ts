...
import { MixpanelAnnotationsSDK } from '../../..'
...
describe('ListAnnotationDirect', async () => {
  ...
  test('direct-list-list_annotation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-list_annotation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MIXPANEL_ANNOTATIONS_TEST_LIST_ANNOTATION_ENTID']
