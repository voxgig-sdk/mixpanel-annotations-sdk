...
describe('ProjectEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.live) {
      return runLiveEntity(setup, {"active":true,"alias":{"field":{}},"fields":[],"name":"project","op":{},"relations":{"ancestors":[]},"key$":"project","name__orig":"project","Name":"Project","name_":"project","name-":"project","NAME":"PROJECT","index$":3}, {"active":true,"entity":"project","key$":"BasicProjectFlow","kind":"basic","name":"BasicProjectFlow","param":{},"step":[]}, 'Project')
    }
...
function basicSetup(extra?: any) {
  ...
  const env = envOverride({
    'MIXPANEL_ANNOTATIONS_TEST_PROJECT_ENTID': idmap,
  ...
  idmap = env['MIXPANEL_ANNOTATIONS_TEST_PROJECT_ENTID']

  const live = 'TRUE' === env.MIXPANEL_ANNOTATIONS_TEST_LIVE

  const transport = createLiveTransport()
  if (live) {
    const rawIds = process.env['MIXPANEL_ANNOTATIONS_TEST_PROJECT_ENTID']
