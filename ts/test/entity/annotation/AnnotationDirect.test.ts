...
describe('AnnotationDirect', async () => {
  ...
  test('direct-load-annotation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-annotation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MIXPANEL_ANNOTATIONS_TEST_ANNOTATION_ENTID']
