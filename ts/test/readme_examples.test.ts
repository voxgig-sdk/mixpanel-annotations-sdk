...
const SDK_NAME = 'MixpanelAnnotationsSDK'
...
const TEST_SEED = {"entity":{"annotation":{"test01":{"id":"test01"}},"annotation_tag":{"test01":{"id":"test01"}},"list_annotation":{"test01":{"id":"test01"}},"project":{"test01":{"id":"test01"}},"tag":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+MixpanelAnnotationsSDK\b/.test(code) ||
    /\bMixpanelAnnotationsSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MixpanelAnnotationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MixpanelAnnotationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
