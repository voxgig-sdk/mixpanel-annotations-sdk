...
require("mixpanel-annotations_types")
...
local MixpanelAnnotationsSDK = {}
MixpanelAnnotationsSDK.__index = MixpanelAnnotationsSDK
...
MixpanelAnnotationsSDK._make_feature = _make_feature


function MixpanelAnnotationsSDK.new(options)
  local self = setmetatable({}, MixpanelAnnotationsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MixpanelAnnotationsSDK:options_map()
...
function MixpanelAnnotationsSDK:get_utility()
  return Utility.copy(self._utility)
end


function MixpanelAnnotationsSDK:get_root_ctx()
  return self._rootctx
end


function MixpanelAnnotationsSDK:prepare(fetchargs)
...
function MixpanelAnnotationsSDK:direct(fetchargs)
...
function MixpanelAnnotationsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MixpanelAnnotationsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MixpanelAnnotationsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MixpanelAnnotationsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MixpanelAnnotationsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MixpanelAnnotationsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MixpanelAnnotationsSDK:Annotation(data)
...
-- Idiomatic facade: client:AnnotationTag():list() / client:AnnotationTag():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MixpanelAnnotationsSDK:AnnotationTag(data)
  local EntityMod = require("entity.annotation_tag_entity")
  if data == nil then
    if self._annotation_tag == nil then
      self._annotation_tag = EntityMod.new(self, nil)
    end
    return self._annotation_tag
  end
  return EntityMod.new(self, data)
end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MixpanelAnnotationsSDK:ListAnnotation(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MixpanelAnnotationsSDK:Project(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function MixpanelAnnotationsSDK:Tag(data)
...
function MixpanelAnnotationsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MixpanelAnnotationsSDK.new(sdkopts)
  sdk.mode = "test"
