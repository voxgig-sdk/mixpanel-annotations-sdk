...
//    only - a typed MixpanelAnnotationsContext there would make every ctx
...
final class MixpanelAnnotationsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MixpanelAnnotationsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MixpanelAnnotationsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MixpanelAnnotationsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MixpanelAnnotationsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MixpanelAnnotationsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MixpanelAnnotationsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MixpanelAnnotationsError(is_string($code) ? $code : '', $msg);
    }
