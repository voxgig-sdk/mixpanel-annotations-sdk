...
// MixpanelAnnotations SDK retry feature
...
class MixpanelAnnotationsRetryFeature extends MixpanelAnnotationsBaseFeature
{
    ...
    public function init(MixpanelAnnotationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MixpanelAnnotationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MixpanelAnnotationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MixpanelAnnotationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
