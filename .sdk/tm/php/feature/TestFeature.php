...
// MixpanelAnnotations SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MixpanelAnnotationsTestFeature extends MixpanelAnnotationsBaseFeature
{
    ...
    public function init(MixpanelAnnotationsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MixpanelAnnotationsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MixpanelAnnotationsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MixpanelAnnotationsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MixpanelAnnotationsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MixpanelAnnotationsParam::call($ctx, $k);
