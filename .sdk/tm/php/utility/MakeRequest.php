...
class MixpanelAnnotationsMakeRequest
{
    public static function call(MixpanelAnnotationsContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new MixpanelAnnotationsResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new MixpanelAnnotationsResponse($fetched);
        } else {
