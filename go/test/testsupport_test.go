...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MIXPANEL_ANNOTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("MIXPANEL_ANNOTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MIXPANEL_ANNOTATIONS_TEST_EXPLAIN"); explain != "" {
