...
import (
	...
	omni "github.com/voxgig-sdk/mixpanel-annotations-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MixpanelAnnotationsError); is {
				out := map[string]any{
					"name":    "MixpanelAnnotationsError",
...
func omniCtx(arg any, client *sdk.MixpanelAnnotationsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MixpanelAnnotationsSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MixpanelAnnotationsError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.MixpanelAnnotationsError{Msg: msg, Code: code}
}
