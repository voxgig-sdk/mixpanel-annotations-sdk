...
func TestTagEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MIXPANEL_ANNOTATIONS_TEST_TAG_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tagRef01Data["project_id"] = setup.idmap["project01"]
...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MIXPANEL_ANNOTATIONS_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MIXPANEL_ANNOTATIONS_TEST_TAG_ENTID"])
